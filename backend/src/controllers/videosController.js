...
import { get_playlist_videos_status,  mark_as_unwatched,   mark_as_watched, check_user_playlist } from '../db/queries.js'; 
...
export const getPlaylistVideosStatus = async (req, res) => {
    ...
    try {
        ...
        const playlistCheck = await pool.query(check_user_playlist, [user_id, playlist_id]);
        if (playlistCheck.rowCount === 0) {
            return res.status(404).json({ error: "Playlist not found in your tracked list" });
        }

        const dbResult = await pool.query( // {videoId, isWatched}
        ...
        if (videoIds.length === 0) {
            return res.status(200).json({ videos: [], totalSec: 0, totalWatchedSeconds: 0 });
        }
        ...
        (videoItems || []).forEach(item => {
            ...
            if (!playlistItems[vId]) {
                ...
                playlistItems[vId] = { title: null, thumbnailURL: null, publishedAt: null };
            }
    ...
    watched_durations_Seconds}]
        */
        ...
        return res.status(200).json({videos: videoArr, totalSec, totalWatchedSeconds});

    } catch (err) {
        console.error('getPlaylistVideosStatus failed:', err?.response?.data || err);
...
export const markAsWatched = async (req, res) => {
    ...
    try {
        const result = await pool.query(
            mark_as_watched,
            [user_id, playlist_id, video_id, duration]
        );
